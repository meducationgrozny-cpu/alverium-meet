    ...
    <div className="flex flex-col h-screen bg-[#000000] text-white font-sans selection:bg-red-900/50">
          ...
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm transition-all duration-300">
            <div className="bg-[#050505] border border-white/10 rounded-2xl w-[420px] p-8 shadow-[0_0_50px_rgba(0,0,0,0.8)] relative flex flex-col gap-5">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 font-light leading-relaxed">Загрузите PDF или презентацию. Ученики увидят материал на главном экране.</p>
              </div>
              <div className="border border-dashed border-white/10 rounded-2xl p-10 flex flex-col items-center justify-center gap-4 hover:border-white/30 hover:bg-white/5 transition-all cursor-pointer group mt-2">
                ...
                <span className="text-[10px] text-gray-400 text-center tracking-widest uppercase leading-loose">Перетащите файлы сюда<br/>или нажмите для выбора</span>
              </div>
      ...
      <footer className="bg-[#050505]/90 backdrop-blur-2xl px-8 py-4 flex justify-between items-center z-20 border-t border-white/5">
        <div className="flex gap-3 w-1/3">
          <button onClick={() => alert('Настройки (в разработке)')} className="flex items-center justify-center w-12 h-12 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 text-gray-400 hover:text-white transition-all duration-300 group">
            <div className="group-hover:rotate-90 transition-transform duration-500"><SettingsIcon /></div>
          </button>
          <button onClick={() => alert('Запись (в разработке)')} className="flex items-center justify-center w-12 h-12 rounded-xl bg-white/5 border border-white/5 hover:bg-red-900/20 text-gray-400 transition-all duration-300 group">
            <RecordIcon />
          </button>
        </div>
        <div className="flex items-center gap-2 bg-[#0a0a0a] px-3 py-2 rounded-2xl border border-white/5 shadow-2xl">
          ...
          <button onClick={() => setIsFilesOpen(true)} className="flex items-center justify-center w-12 h-10 bg-transparent hover:bg-white/10 text-gray-300 hover:text-white rounded-xl transition-all">
            <FileIcon />
          </button>
        </div>
...
// ====================================================
// ВНУТРЕННИЙ КОМПОНЕНТ ДЛЯ ЧТЕНИЯ URL
// ====================================================
function RoomContent() {
  const searchParams = useSearchParams();
  const token = searchParams.get('token'); // Ловим токен из адресной строки!
  const roomUrl = "wss://meet.alverium.ru";

  if (!token) {
    return (
      <div className="text-white flex items-center justify-center h-screen bg-[#000000] font-light tracking-widest text-sm text-gray-500 uppercase text-center p-6">
...
// ====================================================
// КОРНЕВОЙ КОМПОНЕНТ (Обертка Suspense обязательна для Next.js)
// ====================================================
export default function RoomPage() {
  return (
    <Suspense fallback={<div className="text-white flex items-center justify-center h-screen bg-[#000000] font-light tracking-widest text-sm text-gray-500">ЗАГРУЗКА ALVERIUM...</div>}>
      <RoomContent />
    </Suspense>
  );
}
