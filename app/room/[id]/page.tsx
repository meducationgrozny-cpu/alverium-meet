...
function ScreenShareWrapper({ tracks, isHost, room }: { tracks: any[], isHost: boolean, room: any }) {
  ...
  // Автоматический ресайз холста под размер контейнера трансляции
  useEffect(() => {
    const handleResize = () => {
    ...
    setTimeout(handleResize, 100); // Даем React время отрендерить видео
    return () => window.removeEventListener('resize', handleResize);
  }, []);

  // Слушаем рисунки от преподавателя (для учеников)
  useEffect(() => {
  ...
  // Захват мыши/пальца (только для Хоста)
  ...
  return (
    <div className="relative w-full h-full rounded-xl overflow-hidden shadow-2xl bg-black" ref={containerRef}>
      ...
      {/* Панель инструментов поверх экрана (только для препода) */}
      {isHost && (
    ...
    <div className="flex flex-col h-[100dvh] bg-[#000000] text-white font-sans selection:bg-red-900/50 relative">
      ...
      <main className="flex-1 flex overflow-hidden relative">
        <div className="flex-1 relative bg-[#0a0a0a]">
          {isWhiteboardOpen ? (
            ...
            <>
              {/* ВМЕСТО ОБЫЧНОГО GRID LAYOUT ВСТАВЛЯЕМ НАШ СУПЕР-ХОЛСТ */}
      ...
      <footer className="bg-[#050505]/90 backdrop-blur-2xl px-4 md:px-8 py-3 md:py-4 flex justify-between items-center z-20 border-t border-white/5 shrink-0">
        <div className="flex gap-2 md:gap-3 w-auto md:w-1/3">
          ...
          <button onClick={() => alert('Запись (в разработке)')} className="hidden md:flex items-center justify-center w-12 h-12 rounded-xl bg-white/5 border border-white/5 hover:bg-red-900/20 text-gray-400 transition-all"><RecordIcon /></button>
          {isHost && (
